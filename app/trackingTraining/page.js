...
export default function TrackingTrainingPage() {
  ...
  function handleAddSet(exerciseId) {
    updateExercise(exerciseId, (ex) => {
      const nextId = ex.sets && ex.sets.length
    ...
    <div className="w-full min-h-screen py-4 flex flex-col gap-4 items-center">
      <div className="w-full flex flex-col gap-1 px-2">
        <input
          ...
          className="w-full bg-transparent text-3xl font-semibold text-white pb-2 border-b-[3px] border-brand-grey4 focus:outline-none focus:border-brand-purple1 placeholder:text-brand-grey4"
        />
      </div>
      ...
      <div className="w-full flex flex-col gap-4 px-2 pb-10">
        {exercises.length === 0 && (
          <div className="w-full flex flex-col items-center mt-6">
            ...
            <p className="text-xs text-brand-grey4 mt-2">Add your first exercise</p>
          </div>
        )}
