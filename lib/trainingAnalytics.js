...
export function computeMainLift1RMs(logs) {
  ...
  for (const log of logs || []) {
    const workoutDate = parseDate(log.date);
...
export function computeStrongestLiftLast7Days(logs) {
  ...
  lookbackWindow.setDate(now.getDate() - 7);
  lookbackWindow.setHours(0, 0, 0, 0);
...
export function computeMuscleSetDistributionLast8Weeks(logs) {
  ...
  windowStart.setDate(currentWeekStart.getDate() - 56);
  ...
  for (const log of logs || []) {
    ...
    if (workoutDate < windowStart || workoutDate >= currentWeekStart) {
      continue;
    }

    const inLastWeek = workoutDate >= lastWeekStart;
    ...
    for (const ex of log.exercises || []) {
      ...
      totalSetsInWindow[muscle] = (totalSetsInWindow[muscle] || 0) + setsCount;
  ...
  const weeks = 8;
  const arr = Object.entries(totalSetsInWindow).map(([muscle, total]) => ({
    muscle,

    avgSets: Number((total / weeks).toFixed(1)), 
