...
export function computeNutritionStreak(foodLogs, calorieTarget) {
  ...
  for (const log of foodLogs || []) {
    const key =
      log.day ||
      dayKey(log.loggedAt || log.date || log.createdAt);
    if (!key) continue;
    ...
    const cals = Number(
      log.totalCalories ?? log.calories ?? log.kcal ?? 0
    );
...
export function computeLastWeekSummary(exerciseLogs, foodLogs, targets) {
  ...
  for (const log of exerciseLogs || []) {
    ...
    if (!Number.isNaN(d.getTime()) && d >= lastWeekStart && d <= lastWeekEnd) {
      workouts += 1;
    }
  }
  ...
  return {
    ...
    rangeLabel: formatWeekRange(lastWeekStart, lastWeekEnd),
  };
}
...
export function computeMonthlyConsistencyCalendar(
  ...
  for (const log of foodLogs || []) {
    const key =
      log.day || dayKey(log.loggedAt || log.date || log.createdAt);
    if (!key) continue;
